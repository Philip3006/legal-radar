...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  /* App-Bar: Suche fullwidth + Filter-Button rechts (auf allen Screens). */
  ...
  .filter-fab:hover {{ background: var(--surface-2); border-color: var(--border-strong); }}
  .filter-badge {{
  ...
  body.filter-open .filter-sheet {{ display: flex; }}
  ...
  @media (max-width: 720px) {{
    ...
    /* Sheet auf Mobile: Vollbild ohne Backdrop. */
    .filter-sheet {{
      background: #ffffff;
      padding: 0;
      align-items: stretch;
...
(function() {{
  ...
  // --- Deselect: zweiter Klick auf aktiven Chip = zurueck auf Default ---
  // Native Radio-Buttons feuern kein change-Event, wenn schon checked.
  // Wir fangen den Klick auf dem Label ab: wenn das zugehoerige Radio
  // schon aktiv und nicht der Default ist, verhindern wir das Default-
  // Toggling und setzen das Default-Radio derselben Gruppe.
  var DEFAULTS = {{
    fs: 'f-all', ft: 'f-mall', fk: 'f-kall',
    fo: 'f-oscore', fb: 'f-ball'
  }};
  document.querySelectorAll('.filter-row label[for]').forEach(function(lbl) {{
    lbl.addEventListener('click', function(e) {{
      var radio = document.getElementById(lbl.htmlFor);
      if (!radio || !radio.checked) return;
      var def = DEFAULTS[radio.name];
      if (!def || def === radio.id) return;  // schon Default -> nichts
      e.preventDefault();
      var defRadio = document.getElementById(def);
      if (defRadio) {{
